...
- **Advanced networking (5)** and **Admin API / Developer (10)** are small but high-leverage — they
...
| --- | --- | --- | --- |
| `PublishToWeb` | Publishes reports to the public internet, no authentication | **Off** | Anonymous public exposure of report data. The single highest-risk setting in Fabric. If ever enabled, audit existing embed codes — old codes stay live. |
...
| `ExportToCsv` | Export to .csv | **On (scoped)** | Note: labels are **not** enforced on .csv/.txt export paths — this is a genuine label-bypass route. |
...
| `CertifyDatasets` | Who can certify content | **On (scoped)** | Certification is a trust signal — restrict to data stewards or it becomes noise. |
...
| Setting | What it does | Preferred | Why |
| --- | --- | --- | --- |
| `AllowAccessOverPrivateLinks` | Tenant-level Private Link | **On** *(where feasible)* | WAF: "Use private link instead of public endpoints when possible." Requires Azure-side setup and has real limitations — read those first. |
| `BlockAccessFromPublicNetworks` | Block public internet access | **Off until Private Link is proven** | The correct end state for a locked-down tenant, but enabling it before Private Link works will lock everyone out, including you. Sequencing matters. |
...
| Setting | What it does | Preferred | Why |
| --- | --- | --- | --- |
| `AllowServicePrincipalsUseReadAdminAPIs` | SPs can call read-only admin APIs | **On (scoped)** | Required for automated governance/inventory. Grants read of *all* metadata including user names and emails — scope tightly to one automation group. |
...
| `BlockResourceKeyAuthentication` | Block resource-key auth for streaming datasets | **On** | Kills a shared-secret auth path. Straightforward hardening — prefer Entra identities. |
...
| `EimInformationProtectionDownstreamInheritanceSetting` | Apply labels to downstream content | **On** | Prevents label laundering — deriving an unlabelled report from labelled data. |
...
> are a strong control with specific, documented holes — don't treat them as a perimeter.
...
| `OneLakeDiagnosticLogsEUII` | Include end-user identifiers in diagnostic logs | **On** | Investigations need to know *who*. Weigh against privacy/works-council obligations — this is a real trade-off, not a free win. |
...
| Setting | What it does | Preferred | Why |
| --- | --- | --- | --- |
| `SnowflakeSSO`, `RedshiftSSO`, `DremioSSO`, `GoogleBigQuerySSO`, `DenodoForPowerBISSO`, `StarburstAadSSO` | Per-connector SSO | **Off unless that connector is used** | Each sends user token info externally. SSO itself is good practice (MCSB IM-5) — the risk is leaving unused ones on. |
...
| `PowerBIMCP` | Power BI MCP server endpoint (preview) | **Off unless needed** | New surface exposing artifacts to MCP clients. Preview — evaluate deliberately. |
...
| `AllowStoreAOAIDataInOtherRegions` | *Store* AI data outside your boundary | **Off where residency is a requirement** | Storage is a higher bar than transient processing — hold this one harder. |
...
| `AllowCVToExportDataToFileTenant` | Custom visuals can download data | **Off** | Explicitly **bypasses your export restrictions** — the docs say so outright. A hole straight through the export controls. |
...
| `WebModelingTenantSwitch` | Edit semantic models in the service | **Org choice** | Governance preference — browser editing vs. source-controlled Desktop. |
| `EnableReassignDataDomainSwitch` | Admins override workspace domain assignment | **On** | Administrative flexibility, admin-only. |
| `AllowFreeTrial` | Users can start Fabric trials | **Org choice** | Cost governance more than security — trials create real capacity. |
...
| `RealTimeDashboardEmbeddedApp` | Embedded app visuals in Real-Time Dashboards | **Off unless needed** | Embeds app content into dashboards — same class of consideration as web content tiles. |
| `AllowOperatorInsightsTenantSwitch` | Insights for operations agents (preview) | **Org choice** | Explainability over agent activity. |
| `ShowActivatorEntryPointsTenantSwitch` | Show "Set alert" button to all Power BI users | **Org choice** | Cosmetic — only users who can create Fabric items can actually set alerts. |
